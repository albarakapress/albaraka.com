...
async function startServer() {
  ...
  app.delete("/api/customers/:id", (req, res) => {
    try {
      ...
      const orders = db.prepare("SELECT COUNT(*) as count FROM orders WHERE customer_id = ?").get();
      if (orders.count > 0) {
  ...
  app.get("/api/stats", (req, res) => {
    try {
      ...
      const totalRevenue = db.prepare("SELECT SUM(total_amount) as sum FROM orders").get();
      const totalDue = db.prepare("SELECT SUM(due_amount) as sum FROM orders").get();
